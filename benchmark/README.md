...
```
curl -O https://publish.djl.ai/djl-bench/0.21.0/djl-bench_0.21.0-1_all.deb
sudo dpkg -i djl-bench_0.21.0-1_all.deb
```

For macOS, centOS or Amazon Linux 2

You can download djl-bench zip file from [here](https://publish.djl.ai/djl-bench/0.21.0/benchmark-0.21.0.zip).

```
curl -O https://publish.djl.ai/djl-bench/0.21.0/benchmark-0.21.0.zip
unzip benchmark-0.21.0.zip
rm benchmark-0.21.0.zip
sudo ln -s $PWD/benchmark-0.21.0/bin/benchmark /usr/bin/djl-bench
```

For Windows

We are considering to create a `chocolatey` package for Windows. For the time being, you can
download djl-bench zip file from [here](https://publish.djl.ai/djl-bench/0.21.0/benchmark-0.21.0.zip).
