...
ARG version=12.8.1-cudnn-runtime-ubuntu24.04
...
RUN apt-get update && apt-get upgrade -y && apt-get install -y g++ wget unzip openmpi-bin libopenmpi-dev libffi-dev git-lfs rapidjson-dev graphviz cuda-compat-12-8 && \
    scripts/install_python.sh ${python_version} && \
    pip3 cache purge && \
    apt-get clean -y && rm -rf /var/lib/apt/lists/*

# We install TRTLLM separately because it is hosted in a different pypi index
COPY trtllm-container-requirements.txt requirements.txt
RUN pip install tensorrt_llm==0.17.0.post1 --extra-index-url https://pypi.nvidia.com && \
