...
```
curl -O https://publish.djl.ai/djl-serving/djl-serving_0.33.0-1_all.deb
sudo dpkg -i djl-serving_0.33.0-1_all.deb
```

For Windows

We are considering to create a `chocolatey` package for Windows. For the time being, you can
download djl-serving zip file from [here](https://publish.djl.ai/djl-serving/serving-0.33.0.zip).

```
curl -O https://publish.djl.ai/djl-serving/serving-0.33.0.zip
unzip serving-0.33.0.zip
# start djl-serving
serving-0.33.0\bin\serving.bat
```
