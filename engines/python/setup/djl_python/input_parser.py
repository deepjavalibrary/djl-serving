...
def parse_input_with_formatter(batch_input: Input,
    ...
    :param batch_input :(Input) a batch of inputs, each corresponding to a new request
    ...
    for i, item in enumerate(batch):
        try:
            ...
            inputs, param, is_client_side_batch[i] = _parse_inputs_params(
            ...
            if input_format_configs.is_adapters_supported:
                adapters_per_item, found_adapter_per_item = _parse_adapters(
                    inputs, input_map, item, adapter_registry)
...
def _parse_inputs_params(input_map, item, input_format_configs):
    ...
    if "cached_prompt" in input_map:
        param["cached_prompt"] = input_map.pop("cached_prompt")
    if "seed" not in param:
        # set server provided seed if seed is not part of request
        if item.contains_key("seed"):
            param["seed"] = item.get_as_string(key="seed")
    if not "output_formatter" in param:
        param["output_formatter"] = input_format_configs.output_formatter

    if isinstance(inputs, list):
        return inputs, param, True
    else:
        return [inputs], param, False


def _parse_adapters(inputs, input_map, item, adapter_registry) -> (List, bool):
    ...
    if adapters_per_item:
        ...
        adapters_per_item = [""] * len(inputs)

    if len(inputs) != len(adapters_per_item):
        raise ValueError(
