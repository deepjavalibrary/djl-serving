...
class Runner:
    def __init__(self, container):
        ...
        flavor = subprocess.run(['../../serving/docker/scripts/docker_name_builder.sh', container, djl_version], capture_output=True, text=True).stdout.strip()
    ...
    def __exit__(self, *args):
        container = subprocess.run(['docker', 'ps', '-aq'], capture_output=True, text=True).stdout.strip()
        if container != '':
            subprocess.run(['docker', 'rm', '-f', container], shell=True, check=True)
    ...
    def launch(self, env_vars=None):
        ...
        subprocess.run(f'./launch_container.sh {self.image} {model_dir} {self.container} serve -m test=file:/opt/ml/model/test/'.split(), check=True)
