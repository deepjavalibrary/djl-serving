...
def create_session():
    url = "http://127.0.0.1:8080/create_session"
    res = requests.post(url)
    if res.status_code >= 300:
        raise RuntimeError("Create session failed!")
    session_id = res.headers.get("X-Amzn-SageMaker-Session-Id")
    return session_id


def close_session(session_id):
    url = "http://127.0.0.1:8080/close_session"
    res = requests.post(url,
                        headers={"X-Amzn-SageMaker-Session-Id", session_id})
    if res.status_code >= 300:
        raise RuntimeError("Close session failed!")
    return res.headers.get("X-Amzn-SageMaker-Session-Closed") == "true"
...
def test_handler_stateful(model, model_spec):
    ...
    header = {"X-Amzn-SageMaker-Session-Id", session_id}
    if "worker" in spec:
        check_worker_number(spec["worker"])
    stream_values = spec.get("stream", [False, True])
    # dryrun phase
